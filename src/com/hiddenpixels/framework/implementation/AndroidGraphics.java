...
public class AndroidGraphics implements Graphics {
	...
	public void fillOval(int x, int y, int width, int height, int color) {
		paint.setColor(color);
		paint.setStyle(Style.FILL);
		rectF.set(x, y, x+width-1, y+height-1);
	...
	@Override
	public void drawImage(Image image, int x, int y, float sx, float sy, int r) {
		Matrix matrix = new Matrix();
		matrix.postRotate(r);
		matrix.postTranslate(x - image.getWidth() / 2, y - image.getHeight()
				/ 2);
		matrix.postScale(sx, sy, x, y);
